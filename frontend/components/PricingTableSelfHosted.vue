...
export default {
    ...
    async fetch() {
        const table_plans = await this.$axios.get(`${this.$store.state.apiUrl}/selfhosted-plans?locale=${this.$i18n.locale}`)
        this.tablePlans = table_plans.data.data;


        const table_rows = await this.$axios.get(`${this.$store.state.apiUrl}/selfhosted-tables?locale=${this.$i18n.locale}&sort=rank:asc`)
